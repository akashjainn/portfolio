...
function ExecutiveSummaryCard({ project }: ExecutiveSummaryProps) {
  ...
  return (
    <Card className="group flex flex-col h-fit transition-all duration-300 hover:shadow-lg hover:-translate-y-1">
      <CardHeader className="pb-4 space-y-3">
        {/* Header: Category + Timeline + Icon Links */}
        <div className="flex items-start justify-between">
          <div className="flex-1 flex items-center gap-2 flex-wrap">
            <ProjectChip variant="category">{project.category}</ProjectChip>
            <ProjectChip variant="meta" icon={<Clock className="h-3 w-3" />}>
              {project.timeline}
            </ProjectChip>
          </div>

          {/* Icon links (kept minimal, same as before but will migrate to ProjectActions eventually) */}
          <div className="flex gap-1 ml-2 flex-shrink-0">
            ...
            {project.links.repo && (
              <Button size="sm" variant="ghost" className="h-8 w-8 p-0" asChild>
                <a href={project.links.repo} target="_blank" rel="noopener noreferrer" aria-label="View code">
        ...
        {project.metrics.performance && (
          <MetricsRow 
            metrics={[{ label: 'Performance', value: project.metrics.performance, variant: 'quality' }]} 
            layout="wrap" 
          />
        )}
      </CardHeader>
                      ...
                      <div>
                        ...
                        <div className="flex flex-wrap gap-2">
                          {project.techStack.map((tech) => (
                            <ProjectChip key={tech} variant="tech">
                              {tech}
                            </ProjectChip>
                          ))}
                        </div>
