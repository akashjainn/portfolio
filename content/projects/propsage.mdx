...
links:
  demo: "https://propsage-demo.vercel.app"
...
Sports betting props require accurate, real-time pricing that accounts for rapidly changing game conditions. Traditional static odds fail to capture the dynamic nature of live sports events. The challenge was to build:
...
```typescript path=/packages/pricing/monte-carlo.ts start=1
...
```typescript path=/apps/api/websocket.ts start=25
...
```typescript path=/packages/pricing/evidence-processor.ts start=1
...
```typescript path=/apps/api/monitoring.ts start=1
