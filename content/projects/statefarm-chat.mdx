...
links:
  live: "https://www.statefarm.com/customer-care/contact-us"
metrics:
  uptime: "99.98%"
  monthly_users: "2M+"
  message_delivery: "Zero message loss"
...
State Farm needed to modernize their customer-facing support experience with a redesigned chat widget that could handle massive scale while maintaining exceptional reliability. The legacy system had become difficult to maintain, lacked modern design system integration, and struggled with real-time message delivery reliability.
...
The chat widget is embedded directly into State Farm's customer care pages and follows their enterprise design system. The interface prioritizes clarity, accessibility, and intuitive interaction patterns for users across different technical skill levels.

### Widget Integration

![State Farm contact page with embedded chat widget](https://www.statefarm.com/customer-care/contact-us)

The widget is embedded as a persistent panel on the right side of the customer care contact page, providing immediate access to chat support while users explore self-service options.

<div className="my-8">
  <div className="relative rounded-lg overflow-hidden border border-border/50 bg-muted/30">
    <img 
      src="/images/projects/statefarm/widget-integration.png" 
      alt="State Farm contact page with chat widget embedded on right side"
      className="w-full"
    />
  </div>
...
The widget greets users with a clear system message about security and privacy, followed by the virtual assistant introduction. This sets expectations and builds trust before conversation begins.
...
<div className="my-8">
  ...
  <p className="text-sm text-muted-foreground mt-4 text-center">
    Welcome screen emphasizing security with system message, followed by friendly assistant introduction.
  </p>
</div>

### Conversation Flow

Users can ask questions naturally, with the virtual assistant providing guided responses. The interface offers quick-action buttons for common queries while allowing free-form text input.
...
<div className="my-8">
  ...
  <p className="text-sm text-muted-foreground mt-4 text-center">
    Agent provides contextual information with linked resources, helping users self-serve or escalate as needed.
  </p>
</div>

### Session Management

Users can gracefully end conversations with a confirmation dialog. Session state is preserved for transcript logging and analytics.
...
<div className="my-8">
  ...
  <p className="text-sm text-muted-foreground mt-4 text-center">
    Confirmation dialog for session termination, with conversation stored for compliance and future reference.
  </p>
</div>
